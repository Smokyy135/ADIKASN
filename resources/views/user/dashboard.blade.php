...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 min-h-screen flex flex-col">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
                <div class="space-y-2">
                    <div class="flex items-center gap-2">
                        ...
                        <span id="badge-kabupaten" class="hidden bg-gradient-to-r from-emerald-100 to-emerald-200 text-emerald-700 text-xs font-bold px-2 py-0.5 rounded-full min-w-fit">0</span>
                    </div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-2">
                        ...
                        <span id="badge-skpd" class="hidden bg-gradient-to-r from-blue-100 to-blue-200 text-blue-700 text-xs font-bold px-2 py-0.5 rounded-full min-w-fit">0</span>
                    </div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-2">
                        ...
                        <span id="badge-jenis" class="hidden bg-gradient-to-r from-purple-100 to-purple-200 text-purple-700 text-xs font-bold px-2 py-0.5 rounded-full min-w-fit">0</span>
                    </div>
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-2">
                        ...
                        <span id="badge-periode" class="hidden bg-gradient-to-r from-orange-100 to-orange-200 text-orange-700 text-xs font-bold px-2 py-0.5 rounded-full min-w-fit">0</span>
                    </div>
    ...
    window.onload = () => {
        try {
            ...
            if (nameEl) {
                nameEl.innerText = '{{ Auth::user()->nip }}' || 'User';
            }
    ...
    async function loadData() {
        try {
            ...
            const res = await fetch('{{ route("user.files") }}?t=' + new Date().getTime());
            if (!res.ok) {
    ...
    async function populateDropdowns() {
        try {
            console.log('Fetching categories from:', '{{ route("user.categories") }}');
            
            // Add timestamp to prevent caching
            const res = await fetch('{{ route("user.categories") }}?t=' + new Date().getTime());
            if (!res.ok) {
            ...
            if (categories.kabupaten && Array.isArray(categories.kabupaten) && categories.kabupaten.length > 0) {
                ...
                document.getElementById('badge-kabupaten').innerText = categories.kabupaten.length;
                document.getElementById('badge-kabupaten').classList.remove('hidden');
            } else {
                console.log('No kabupaten data available');
                document.getElementById('badge-kabupaten').classList.add('hidden');
            }
            ...
            if (categories.skpd && Array.isArray(categories.skpd) && categories.skpd.length > 0) {
                ...
                document.getElementById('badge-skpd').innerText = categories.skpd.length;
                document.getElementById('badge-skpd').classList.remove('hidden');
            } else {
                document.getElementById('badge-skpd').classList.add('hidden');
            }
            ...
            if (categories.jenisData && Array.isArray(categories.jenisData) && categories.jenisData.length > 0) {
                ...
                document.getElementById('badge-jenis').innerText = categories.jenisData.length;
                document.getElementById('badge-jenis').classList.remove('hidden');
            } else {
                document.getElementById('badge-jenis').classList.add('hidden');
            }
            ...
            if (categories.periode && Array.isArray(categories.periode) && categories.periode.length > 0) {
                ...
                document.getElementById('badge-periode').innerText = categories.periode.length;
                document.getElementById('badge-periode').classList.remove('hidden');
            } else {
                document.getElementById('badge-periode').classList.add('hidden');
            }

            console.log('Dropdowns populated successfully');
            
            // Recreate icons after adding badges
            if (window.lucide) {
                lucide.createIcons();
            }
        } catch (error) {
    ...
    function downloadSelectedFile() {
        ...
        window.location.href = `{{ route('user.file.download', ':id') }}`.replace(':id', selectedFileData.id);
    ...
    async function exportPDF() {
        ...
        window.location.href = `{{ route('user.file.export-pdf', ':id') }}`.replace(':id', selectedFileData.id);
    }
