...
<head>
    ...
    <style>
        ...
        .dropzone-active { border-color: #3b82f6 !important; background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%) !important; }
        .toast { animation: slideIn .3s ease; }
        @keyframes slideIn { from { transform: translateY(20px); opacity: 0; } to { transform: translateY(0); opacity: 1; } }
        .progress-bar { transition: width 0.3s ease; background: linear-gradient(90deg, #3b82f6 0%, #1d4ed8 100%); }
...
<body class="bg-gradient-to-br from-slate-50 via-blue-50 to-slate-100 min-h-screen flex flex-col">
    <nav class="gradient-header border-b border-slate-200/50 px-4 sm:px-8 h-16 flex justify-between items-center sticky top-0 z-40 shadow-lg backdrop-blur-sm">
        <div>
            <p class="text-base font-black text-white">⚙️ Panel Admin</p>
            <p class="text-xs text-blue-100 hidden sm:block">BKPSDM Kabupaten Tabalong</p>
        </div>
    ...
    <main class="flex-1 p-4 sm:p-6 lg:p-8 space-y-6 max-w-5xl w-full mx-auto">
        <div class="mb-2">
            <h1 class="text-3xl sm:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 mb-2">Kelola Sistem</h1>
            <p class="text-slate-600 text-sm font-semibold">Upload file, kelola kategori, dan administrasi data pengguna</p>
        </div>

        <!-- UPLOAD FILE -->
        <div id="section-upload" class="bg-white rounded-3xl border-2 border-slate-200 shadow-lg p-6 sm:p-8 backdrop-blur-sm">
            <h2 class="text-lg font-black text-slate-900 mb-6 flex items-center gap-3">
                <div class="bg-gradient-to-br from-blue-100 to-blue-50 p-2.5 rounded-2xl">
                    <i data-lucide="upload-cloud" class="w-6 h-6 text-blue-600"></i>
                </div>
                Unggah File untuk Pengguna
            </h2>
            <div id="dropzone" class="border-3 border-dashed border-slate-300 rounded-3xl p-8 sm:p-12 text-center hover:border-blue-500 hover:bg-gradient-to-br hover:from-blue-50 hover:to-indigo-50 transition-all duration-300 cursor-pointer group">
                <div class="flex flex-col items-center">
                    <div class="bg-gradient-to-br from-blue-100 to-indigo-100 p-5 rounded-full mb-4 group-hover:scale-110 transition-transform duration-300">
                        <i data-lucide="upload-cloud" class="w-10 h-10 text-blue-600"></i>
                    </div>
                    <p class="text-lg font-black text-slate-800">Tarik &amp; Lepas File di Sini</p>
                    <p class="text-sm text-slate-600 mt-2">atau klik untuk memilih file</p>
                    <p class="text-xs text-blue-600 mt-3 font-bold">📊 Excel (.xls/.xlsx) • 📄 PDF • 📝 Word (.doc/.docx) • 🖼️ Gambar (Maks. 5MB)</p>
            ...
            <div id="upload-progress" class="hidden mt-6">
                <div class="flex justify-between text-xs text-slate-600 mb-2">
                    <span id="upload-filename" class="font-semibold">Membaca file...</span>
                    <span id="upload-percent" class="font-bold text-blue-600">0%</span>
                </div>
                <div class="bg-slate-200 rounded-full h-3 overflow-hidden shadow-md">
                    <div id="progress-bar" class="progress-bar h-3 rounded-full" style="width:0%"></div>
                </div>
            </div>

            <div id="upload-form" class="hidden mt-7 space-y-4 border-t-2 border-slate-100 pt-6">
                <p class="text-xs font-black text-slate-600 uppercase tracking-wider">🏷️ Label File (Opsional — untuk filter pengguna)</p>
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                        <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Kabupaten</label>
                        <select id="upload-kabupaten" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                            <option value="">-- Pilih --</option>
                        </select>
                    </div>
                    <div>
                        <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Satuan Unit Kerja</label>
                        <select id="upload-skpd" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                            <option value="">-- Pilih --</option>
                        </select>
                    </div>
                    <div>
                        <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Jenis Data</label>
                        <select id="upload-jenis" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                            <option value="">-- Pilih --</option>
                        </select>
                    </div>
                    <div>
                        <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Periode</label>
                        <select id="upload-periode" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                            <option value="">-- Pilih --</option>
                        </select>
                    </div>
                </div>
                <div>
                    <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Deskripsi File</label>
                    <input type="text" id="upload-desc" placeholder="Contoh: Data kepegawaian bulan Maret 2026" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                </div>
                <div class="flex flex-col sm:flex-row gap-3 pt-3">
                    <button id="btn-confirm-upload" onclick="confirmUpload()" class="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3.5 rounded-xl text-sm font-bold hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-200">
                        <i data-lucide="check" class="w-5 h-5"></i> <span>✓ Simpan &amp; Publikasikan</span>
                    </button>
                    <button id="btn-cancel-upload" onclick="cancelUpload()" class="px-5 py-3.5 bg-slate-200 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-300 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
                        Batal
                    </button>
                </div>
            </div>
        ...
        <!-- FILE TERSIMPAN -->
        <div id="section-files" class="bg-white rounded-3xl border-2 border-slate-200 shadow-lg p-6 sm:p-8 backdrop-blur-sm">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
                <h2 class="text-lg font-black text-slate-900 flex items-center gap-3">
                    <div class="bg-gradient-to-br from-emerald-100 to-emerald-50 p-2.5 rounded-2xl">
                        <i data-lucide="folder-open" class="w-6 h-6 text-emerald-600"></i>
                    </div>
                    File Tersimpan
                    <span id="file-count-badge" class="hidden bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 text-xs font-black px-3 py-1 rounded-full"></span>
                </h2>
                <button onclick="clearAllFiles()" id="btn-clear-all" class="hidden text-sm text-red-600 hover:text-red-800 font-bold px-4 py-2 hover:bg-red-50 rounded-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto">
                    🗑️ Hapus Semua
                </button>
            </div>
            <div id="file-history" class="space-y-3 max-h-[480px] overflow-y-auto pr-2"></div>
        </div>

        <!-- KELOLA DROP LIST -->
        <div id="section-dropdown" class="bg-white rounded-3xl border-2 border-slate-200 shadow-lg p-6 sm:p-8 backdrop-blur-sm">
            <h2 class="text-lg font-black text-slate-900 mb-6 flex items-center gap-3">
                <div class="bg-gradient-to-br from-violet-100 to-violet-50 p-2.5 rounded-2xl">
                    <i data-lucide="settings-2" class="w-6 h-6 text-violet-600"></i>
                </div>
                Tambah &amp; Kelola Kategori
            </h2>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div class="space-y-4">
                    <p class="text-xs font-black text-slate-700 uppercase tracking-wider">➕ Tambah Kategori Baru</p>
                    <div>
                        <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Kabupaten</label>
                        <input type="text" id="add-kabupaten" placeholder="Contoh: Kabupaten Tabalong" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                    </div>
                    <div>
                        <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Satuan Unit Kerja</label>
                        <input type="text" id="add-skpd" placeholder="Contoh: Dinas Kesehatan" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all hover:border-blue-300">
                    </div>
                    <div>
                        <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Jenis Data</label>
                        <input type="text" id="add-gol" placeholder="Contoh: Berdasarkan Jabatan" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all hover:border-blue-300">
                    </div>
                    <div>
                        <label class="text-xs font-bold text-slate-700 uppercase tracking-wider">Periode</label>
                        <input type="text" id="add-periode" placeholder="Contoh: 2026" class="w-full mt-2 px-4 py-3 bg-gradient-to-br from-slate-50 to-slate-100 border-2 border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all hover:border-blue-300">
                    </div>
                    <button id="btn-add-category" onclick="addDataManual()" class="w-full px-4 py-3.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl text-sm font-bold hover:from-blue-700 hover:to-indigo-700 transition-all duration-300 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-200">
                        <i data-lucide="plus" class="w-5 h-5"></i> Tambah Kategori
                    </button>
                </div>
                <div>
                    <p class="text-xs font-black text-slate-700 uppercase tracking-wider mb-4">📋 Daftar Kategori</p>
                    <div id="filter-lists" class="space-y-3 max-h-72 overflow-y-auto pr-2"></div>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        window.onload = () => {
            try {
                ...
                if (nameEl) {
                    nameEl.innerText = '{{ Auth::user()->nip }}' || 'Admin';
                }
        ...
        async function loadFilterLists() {
            try {
                ...
                const res = await fetch('{{ route("admin.categories") }}?t=' + new Date().getTime());
                if (!res.ok) {
        ...
        async function deleteFilterItem(id, type, name) {
            ...
            try {
                const res = await fetch('{{ route("admin.category.delete", ":id") }}'.replace(':id', id), {
                    method: 'DELETE',
        ...
        async function addDataManual() {
            ...
            try {
                ...
                if (kabupaten) {
                    try {
                        const res = await fetch('{{ route("admin.category.add") }}', {
                            method: 'POST',
                            headers: {
        ...
        async function loadFileHistory() {
            try {
                const res = await fetch('{{ route("admin.files") }}');
                if (!res.ok) {
        ...
        async function deleteFile(id) {
            ...
            try {
                const res = await fetch(`{{ route("admin.file.delete", ":id") }}`.replace(':id', id), {
                    method: 'DELETE',
                    headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}
                });
                const data = await res.json();
        ...
        async function clearAllFiles() {
            ...
            try {
                const res = await fetch('{{ route("admin.files.clear") }}', {
                    method: 'DELETE',
                    headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}
                });
                const data = await res.json();
